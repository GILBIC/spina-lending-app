...
def _method(repository):
    ...
    if method is None:
        pytest.fail("CIF review confirmation repository is not implemented", pytrace=False)
    return method
...
def test_confirmation_persists_exact_snapshot_server_witness_and_time_without_side_effects(
    ...
    assert record.applicant_confirmation_evidence_reference == row[
...
def test_stale_expected_information_is_rejected(connection, monkeypatch, field):
    ...
    with pytest.raises(_error(repository, "ClientCifConflict"), match="refresh|changed"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"full_name": "Name", "phone_number": "1234567", "email": None,
         "present_address": "Address", "extra": "forbidden"},
        {"full_name": 42, "phone_number": "1234567", "email": None,
         "present_address": "Address"},
    ],
)
def test_malformed_snapshot_is_rejected_without_writes(connection, monkeypatch, expected):
    repository = _repository(connection, monkeypatch)
    case = _seed(connection)
...
def test_evidence_reference_must_be_nonblank_text(
    connection, monkeypatch, reference
):
    repository = _repository(connection, monkeypatch)
    case = _seed(connection)
    with pytest.raises(ValueError):
...
@pytest.mark.parametrize(
    "scenario",
    ["wrong_client", "wrong_cif", "noncurrent", "superseded", "blocked", "closed",
     "requirements_incomplete", "under_verification", "wrong_link", "mismatched_stage"],
)
def test_new_confirmation_requires_fresh_paired_eligible_source(
    ...
    if scenario == "wrong_client":
        ...
        connection.execute("update lending.client_cif_versions set is_current=false where id=%s", (case["cif"],))
    elif scenario == "superseded":
        connection.execute("update lending.client_cif_versions set status='superseded' where id=%s", (case["cif"],))
    elif scenario in {"blocked", "closed"}:
        connection.execute("update lending.clients set status=%s where id=%s", (scenario, case["client"]))
    elif scenario in {"requirements_incomplete", "under_verification"}:
        connection.execute("update lending.client_onboarding_applicants set status=%s where id=%s", (scenario, case["applicant"]))
    elif scenario == "wrong_link":
        connection.execute("update lending.client_onboarding_applicants set promoted_client_id=%s where id=%s", (case["other"], case["applicant"]))
...
def test_incomplete_stored_information_cannot_be_confirmed(
    ...
    with pytest.raises(_error(repository, "ClientCifConflict"), match="incomplete|invalid"):
        _confirm(repository, connection, case)
    assert _rows(connection, case["client"]) == []
...
def test_insert_failure_rolls_back_without_partial_effects(connection, monkeypatch):
    ...
    ).format(sql.Identifier(trigger), sql.Literal(str(case["cif"])), sql.Identifier(trigger)))
    before = _summary_state(connection, case)
    with pytest.raises(psycopg.errors.CheckViolation, match="SYNTHETIC CIF confirmation failure"):
        _confirm(repository, connection, case)
    ...
    with psycopg.connect(runtime_url, autocommit=True, row_factory=dict_row) as observer:
        ...
        def acquire():
            if get_ident() == owner_thread:
                ...
                with psycopg.connect(runtime_url, row_factory=dict_row, options=options, connect_timeout=5) as contender:
        ...
        try:
            ...
            while owner.info.backend_pid not in observer.execute(
                "select pg_blocking_pids(%s) as blockers", (pid,)
            ).fetchone()["blockers"]:
                assert monotonic() < deadline, "Confirmation did not wait on the CIF review lock"
                Event().wait(0.02)
            owner_confirmation = None
            if winner == "correction":
                repository.correct_draft_information(
                    actor_user_id=case["actor"], client_id=case["client"],
                    cif_version_id=case["cif"], expected_information=payload["expected_information"],
                    corrected_information=corrected, reason="Synthetic concurrent correction",
                )
            else:
            ...
            if winner == "correction":
                ...
                assert rows[0]["review_cycle_number"] == owner_confirmation.review_cycle_number

...
from gilbic_backend.cross_remittance_repository import CrossRemittanceTargetRecord
...
class FakeCrossRemittances:
    ...
    def preview(
        ...
        *,
        ...
        assert collector_user_id == COLLECTOR_USER_ID
        assert recipient_user_id == ASSIGNED_COLLECTOR_ID
        assert collection_date == COLLECTION_DATE
        return _summary()
...
def test_cross_remittance_targets_only_show_assigned_collectors_with_items() -> None:
    client, _ = _client()
...
def test_cross_remittance_submission_explains_one_tap_adoption() -> None:
    ...
    response = client.post(
        ...
        headers=_headers(),
        json={
            "recipient_user_id": str(ASSIGNED_COLLECTOR_ID),
            "collection_date": COLLECTION_DATE.isoformat(),
            "note": "Cash handed to assigned collector",
        },
    )

    assert response.status_code == 201
    data = response.json()["data"]
    assert data["recipient_role"] == "Assigned Collector"
    assert data["status"] == "submitted"
    assert "without creating another transaction" in data["acceptance_message"]
    assert remittances.submit_request == (
        COLLECTOR_USER_ID,
        ASSIGNED_COLLECTOR_ID,
        COLLECTION_DATE,
        "Cash handed to assigned collector",
    )

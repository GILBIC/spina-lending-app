...
from first_loan_disclosure_fixtures import (
    component_values, disclosure_values, projection_references, review_values,
    terms_values,
)
from gilbic_backend.first_loan_terms import (
    FirstLoanTerms, generate_first_loan_schedule, snapshot_digest,
)
...
@pytest.mark.parametrize("bad", [True, False, 10.0, "10.001", "-1.00", "NaN",
                                  "Infinity", "10000000000000000.00", None,
...
@pytest.mark.parametrize("field", ["total_upfront_deductions", "net_proceeds",
                                  "total_scheduled_payable", "principal"])
...
def test_first_loan_cannot_hide_a_renewal_offset():
    with pytest.raises(ValueError):
        subject().parse_components(component_values(
            renewal_offset="1.00", total_upfront_deductions="11.00", net_proceeds="989.00"))


def test_positive_net_cash_and_principal_are_required():
    for data in [component_values(dst_upfront="1000.00",
                                 total_upfront_deductions="1000.00", net_proceeds="0.00"),
                 component_values(principal="0.00", dst_upfront="0.00",
                                  total_upfront_deductions="0.00", net_proceeds="0.00",
...
def test_exact_projection_never_mutates_inputs():
    ...
    result = subject().project_components(subject().parse_components(data), references=references)
    assert result == data
...
def test_projection_requires_exact_supplied_references(change):
    ...
    with pytest.raises(ValueError):
        subject().project_components(subject().parse_components(component_values()),
                                     references=references)


def test_maximum_exact_cent_value_and_local_context_are_preserved():
    data = component_values(principal="9999999999999999.99", contractual_interest="0.00",
                            dst_upfront="0.00", total_upfront_deductions="0.00",
    ...
    with localcontext() as context:
        context.prec = 6
        before = context.copy()
        result = m.project_components(m.parse_components(data), references=projection_references())
        assert result == data
...
def test_review_digest_is_key_stable_exact_and_preserves_unicode_convention():
    m = subject()
    data = {"components": component_values(), "note": "Peña", "source_reference": "0010"}
    ...
    for spelling in [10, "10", "10.0", "10.00", Decimal("10.00")]:
        ...
        assert m.canonical_review_digest(dict(reversed(list(changed.items())))) == expected
...
def test_digest_normalizes_nested_terms_money_but_not_reference_strings_or_rate_scale():
    ...
    assert m.canonical_review_digest({"reference": "0010"}) != m.canonical_review_digest({"reference": "10"})
    assert m.canonical_review_digest({"rate": Decimal("0.01234567890")}) == snapshot_digest({"rate": "0.01234567890"})


@pytest.mark.parametrize("bad", [float("nan"), 10.0, Decimal("sNaN"), {1: "bad"}, {"x"}])
...
def test_model_is_frozen_and_rejects_unplanned_fields():
    ...
    for field in ["approved", "reviewed_by_user_id", "reviewed_at", "client_id", "approval_ready"]:
        with pytest.raises(ValueError):
            m.DisclosureReviewRequest.model_validate(review_values(**{field: "spoofed"}))
...
@pytest.mark.parametrize("kind,timing", [("grt_recovery", "upfront"), ("dst", "repayments"),
                                       ("other_upfront", "repayments"), ("other_scheduled", "upfront")])
...
def test_every_existing_deduction_maps_once_by_identity_and_amount():
    for change in ["missing", "extra", "wrong_amount", "renamed", "grt"]:
        ...
        if change == "extra":
            data["terms"]["deductions"].append({"code": "another", "amount": "10.00", "authority_reference": "SYN"})
...
def test_explicit_supported_zero_requires_review_reason_not_assumed_exemption():
    m = subject()
    data = review_values()
    assert m.DisclosureReviewRequest.model_validate(data).components.grt_in_repayments == 0
...
@pytest.mark.parametrize("field", ["request_id", "cif_version_id", "dst_rule_id", "grt_rule_id",
                                  "supersedes_calculation_id", "borrower_charge_basis", "charge_items"])
...
def test_context_digest_shape_is_exact(bad):
    with pytest.raises(ValueError):
        subject().DisclosureReviewRequest.model_validate(review_values(expected_context_digest=bad))


@pytest.mark.parametrize("field,value", [("calculation_method", "automatic_v1"),
    ("support_base64", "%%bad"), ("support_base64", ""),
    ("support_media_type", "text/html"), ("review_rationale", "   "),
    ("review_rationale", "a" * 2001)])
def test_review_metadata_is_bounded_explicit_and_not_a_provider_claim(field, value):
    with pytest.raises(ValueError):
        subject().DisclosureReviewRequest.model_validate(review_values(**{field: value}))


def test_independent_disclosure_values_require_their_own_references():
    m = subject()
    for name in ["amount_financed", "finance_charge_total", "non_finance_charge_total", "effective_interest_rate"]:
        data = review_values()
...
def test_public_snapshot_is_an_explicit_allowlist_and_preserves_unknown_values():
    m = subject()
    data = review_values(disclosure_values=disclosure_values(
...
def test_eir_keeps_supplied_precision_and_unit_without_money_rounding():
    data = review_values(disclosure_values=disclosure_values(
        effective_interest_rate="0.1234567890", effective_interest_rate_reference="SYN EIR",
        rate_period="annual", calculation_method="SYN-METHOD"))
    m = subject()
    result = m.public_financial_snapshot(m.DisclosureReviewRequest.model_validate(data).model_dump(mode="json"))
...
def test_compatible_regular_rows_are_unchanged():
    ...
    assert m.require_component_compatibility(terms, rows, m.parse_components(component_values())) is None
...
def test_positive_unsupported_scheduled_components_cannot_relabel_interest(field):
    ...
    terms = FirstLoanTerms.model_validate(terms_values())
    rows = generate_first_loan_schedule(terms)
    components = m.parse_components(component_values(contractual_interest="190.00", **{field: "10.00"}))
...
def test_reviewable_positive_grt_does_not_mean_compatible_for_issuance():
    m = subject()
    data = review_values()
    data["components"].update(grt_in_repayments="10.00", total_scheduled_payable="1210.00")
    data["charge_items"].append(dict(item_id="grt", kind="grt_recovery", timing="repayments",
                                     amount="10.00", support_section_reference="SYN grt"))
    data["borrower_charge_basis"]["grt_zero_reason"] = None
    reviewed = m.DisclosureReviewRequest.model_validate(data)
    with pytest.raises(ValueError, match="component integration required"):
        m.require_component_compatibility(reviewed.terms, generate_first_loan_schedule(reviewed.terms), reviewed.components)


def test_7x7_components_use_existing_signed_schedule_interest():
    m = subject()
    terms = FirstLoanTerms.model_validate(terms_values(product_code="seven_by_seven",
        contractual_interest=None, interest_rate_percent=None, daily_interest_per_1000="5.00",
        installment_amount="105.00", installment_count=None))
    rows = generate_first_loan_schedule(terms)
    components = m.parse_components(component_values(contractual_interest="50.00", total_scheduled_payable="1050.00"))
...
def test_empty_or_tampered_schedule_and_inconsistent_term_binding_are_rejected():
    ...
    changed = (replace(rows[0], due_date=rows[0].due_date + timedelta(days=1)), *rows[1:])
...
def test_compatibility_rejects_float_values_even_when_dataclass_equality_matches():
    ...
    with pytest.raises(ValueError):
        m.require_component_compatibility(terms, altered, m.parse_components(component_values()))
...
def test_compatibility_revalidates_preconstructed_terms():
    ...
    with pytest.raises(ValueError):
        m.require_component_compatibility(poisoned, rows, m.parse_components(component_values()))


def test_projection_revalidates_immutable_model_copies():
    m = subject()
    components = m.parse_components(component_values()).model_copy(update={"dst_upfront": 10.0})
    with pytest.raises(ValueError):
...
def test_component_overflow_is_rejected_without_changing_decimal_context():
    m = subject()
    data = component_values(principal="9999999999999999.99", dst_upfront="0.00",
                            total_upfront_deductions="0.00", net_proceeds="9999999999999999.99",
                            contractual_interest="0.01", total_scheduled_payable="10000000000000000.00")
    with localcontext() as context:
        context.prec = 6
        with pytest.raises(ValueError):
...
def test_more_than_thirty_charge_items_fail_without_truncating():
    data = review_values()
    data["charge_items"].extend(dict(item_id=f"other-{i}", kind="other_scheduled",
        timing="repayments", amount="0.00", support_section_reference="SYN") for i in range(30))
    with pytest.raises(ValueError):
        subject().DisclosureReviewRequest.model_validate(data)
...
def test_zero_money_has_one_canonical_spelling():
    ...
    for value in ["0", "-0.00", Decimal("-0E-50")]:
        assert m.parse_components(component_values(grt_in_repayments=value)).model_dump(mode="json")["grt_in_repayments"] == "0.00"
...
def test_disclosed_rates_reject_inexact_or_unbounded_representation(value):
    data = review_values(disclosure_values=disclosure_values(
        effective_interest_rate=value, effective_interest_rate_reference="SYN EIR",
        rate_period="annual", calculation_method="SYN METHOD"))
    with pytest.raises(ValueError):
        subject().DisclosureReviewRequest.model_validate(data)
...
def test_support_decoded_size_limit_is_not_just_base64_length():
    ...
    assert m.DisclosureReviewRequest.model_validate(review_values(support_base64=allowed))
    with pytest.raises(ValueError):
...
def test_first_loan_public_contract_cannot_enable_issuance():
    ...
    assert not ({"ready", "approval_ready", "approved", "tax_due", "journal_id"} & set(public))

...
def require_arial_names(names) -> list[str]:
    ...
    if not observed or any(
        not isinstance(n, str) or _name(n) not in _ARIAL_NAMES for n in observed
    ):
        ...
        for number, page in enumerate(doc, start=1):
            ...
            for resource in page.get_fonts(full=True):
                ...
                if not xref or not doc.extract_font(xref)[3]:
                    raise ValueError(
                        f"Page {number}: font is not embedded: {base_font}"
                    )
                available.add(normalized)
                embedded.add(xref)
            if not {_name(n) for n in visible}.issubset(available):
                raise ValueError(
                    f"Page {number}: visible Arial font has no embedded resource"
                )
    return {
        "status": "PASS",
        "fonts_observed": sorted(names),

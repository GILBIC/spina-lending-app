...
from .v1_tax_recoverable_refund_repository import (
    ...
    V1TaxRecoverableRefundCandidate,
...
class RecordV1TaxRecoverableRefundEvidenceRequest(StrictV1TaxRecoverableRefundRequest):
    ...
    def normalize_text(cls, value: str) -> str:
        ...
        if not normalized:
            raise ValueError(
                "Required Tax Recoverable refund evidence text cannot be blank."
            )
        return normalized
...
class PostV1TaxRecoverableRefundRequest(StrictV1TaxRecoverableRefundRequest):
    ...
    def exact_currency_cents(cls, value: Decimal) -> Decimal:
        if value != value.quantize(Decimal("0.01")):
            raise ValueError(
                "Expected Tax Recoverable refund amount must use exact currency-cent precision."
            )
        return value


def v1_tax_recoverable_refund_repository_dependency() -> (
    PostgresV1TaxRecoverableRefundRepository
):
...
def _item_payload(item: V1TaxRecoverableRefundItem) -> dict[str, object]:
    return {
        ...
        "refund_posting_id": str(item.refund_posting_id)
        if item.refund_posting_id
        else None,
...
def _candidate_payload(item: V1TaxRecoverableRefundCandidate) -> dict[str, object]:
    return {
        "adjustment_posting_id": str(item.adjustment_posting_id),
        "adjustment_evidence_id": str(item.adjustment_evidence_id),
        "tax_type": item.tax_type,
        ...
        "loan_id": str(item.loan_id),
        "client_id": str(item.client_id),
        "recoverable_amount": format(item.recoverable_amount, ".2f"),
        "minimum_refund_date": item.minimum_refund_date.isoformat(),
...
def create_v1_tax_recoverable_refund_router() -> APIRouter:
    ...
    def list_v1_tax_recoverable_refunds(
        refund_status: Literal[
            "all", "ready", "prepared", "realized", "blocked"
        ] = Query(default="all"),
            ...
            "data": {
                ...
                "items": [
                    ...
                    for item in tax.list_items(
                        status=refund_status, limit=limit, offset=offset
                    )
                ],
                "refund_candidates": [
                    _candidate_payload(item)
                    for item in tax.list_refund_candidates(limit=limit, offset=offset)
                ],
                "permissions": {
                ...
                "notice": (
                    ...
                    "Tax-credit application remains a separate exact full-only path. Partial and mixed recoverable realization remain disabled; automatic source posting remains disabled."
                ),
            },
        }

    @router.post(
        "/api/mobile/v1/management/financial-accounting/tax/recoverable-refunds/evidence",
    ...
    @router.post(
        "/api/mobile/v1/management/financial-accounting/tax/recoverable-refunds/{refund_evidence_id}/prepare"
    )
    @router.post(
    ...
    def prepare_v1_tax_recoverable_refund(
        ...
        _require_confirmation(
            body.confirm, "preparing a protected Tax Recoverable refund journal"
        )
        try:
            tax.prepare(
                refund_evidence_id=refund_evidence_id, actor_user_id=actor.user_id
            )
    ...
    @router.post(
        "/api/mobile/v1/management/financial-accounting/tax/recoverable-refunds/{refund_evidence_id}/post"
    )
    @router.post(
    ...
    def post_v1_tax_recoverable_refund(
        ...
        _require_confirmation(
            body.confirm, "posting a protected Tax Recoverable refund journal"
        )
        try:
            tax.post(

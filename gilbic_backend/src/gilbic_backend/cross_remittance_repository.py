        ...
        *,
        ...
        with open_connection() as connection:
            with connection.cursor(row_factory=dict_row) as cursor:
                ...
                self._verify_assigned_collector(
                    cursor,
        ...
        *,
        ...
        if collector_user_id == recipient_user_id:
            raise RemittanceRecipientInvalid(
                "The payment is already assigned to the signed-in collector."
            )
                ...
                with connection.cursor(row_factory=dict_row) as cursor:
                    cursor.execute(
                        ...
                        (
                            ...
                            f"{recipient_user_id}:{collection_date}",
                        ),
                    )
                    collector_name = PostgresRemittanceRepository._user_name(
                        cursor,
                    ...
                    recipient_name = self._verify_assigned_collector(
                        cursor,
                        recipient_user_id=recipient_user_id,
                    )
                    items = self._eligible_items(
                    ...
                    if not summary.items:
                        raise RemittanceEmpty(
                            "There are no unlocked other-area payments for this assigned collector."
                        )
                            ...
                            Jsonb(
                                {
                                    ...
                                    "assigned_collector_user_id": str(
                                        recipient_user_id
                                    ),
    ...
    def _eligible_items(
        ...
        *,
        ...
        cursor.execute(
            ...
            where transaction.collector_user_id = %s
              and transaction.assigned_collector_user_id = %s
              and transaction.collection_origin = 'cross_collector'
              and transaction.collection_date = %s
              and transaction.remittance_id is null
              and transaction.is_locked = false
            ...
            {lock_clause}
            """,
            (collector_user_id, recipient_user_id, collection_date),
        )
        return tuple(

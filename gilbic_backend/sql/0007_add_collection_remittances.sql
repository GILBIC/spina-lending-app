...
CREATE TABLE IF NOT EXISTS lending.collection_remittances (
    ...
    CHECK (transaction_count = payment_count + unable_to_pay_count),
    CHECK (covered_payment_count <= payment_count),
    CHECK (
...
CREATE TABLE IF NOT EXISTS lending.collection_supervisor_adjustments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    transaction_id UUID NOT NULL
        REFERENCES lending.collection_transactions(id) ON DELETE RESTRICT,
    ...
    adjustment_type TEXT NOT NULL
        CHECK (adjustment_type IN ('increase', 'decrease', 'note_correction', 'coverage_correction')),
...
CREATE INDEX IF NOT EXISTS lending_collection_supervisor_adjustments_transaction_idx
    ON lending.collection_supervisor_adjustments (transaction_id, created_at DESC);
...
COMMENT ON TABLE lending.collection_remittances IS
    'Server-calculated collector cash handovers. Submission locks every included collection transaction.';
COMMENT ON TABLE lending.collection_transaction_edits IS
    'Append-only audit snapshots for corrections made by the original collector before remittance.';
COMMENT ON TABLE lending.collection_supervisor_adjustments IS
    'Append-only authorized adjustments for already locked and remitted collection transactions.';

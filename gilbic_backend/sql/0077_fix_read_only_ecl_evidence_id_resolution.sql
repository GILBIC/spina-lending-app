...
-- Disposable PostgreSQL proof found an ambiguity between the PL/pgSQL
-- evidence_id variable and the unnest column alias used while canonicalizing
-- the exact forward-looking evidence set. Replace only the protected v1
-- implementation with an explicitly qualified alias. The public 0076 wrapper,
-- immutable snapshot schema, read-only queue, and posting boundary stay intact.

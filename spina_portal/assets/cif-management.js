...
function draftFormMarkup({ clientId = '', record = null, session }) {
  ...
  if (record && String(record.lifecycle_state || '').toLowerCase() !== 'draft') return '';

  const draft = record || {};
  const present = draft.present_address || {};
  const permanent = draft.permanent_address || {};
  const livelihood = draft.livelihood_profile || {};
  const isEdit = Boolean(draft.cif_id);
  const signatureRequired = isEdit && draft.has_client_signature ? ' required' : '';
  const signatureMessage = signatureRequired
    ? 'This draft already has a protected signature reference. Re-enter both the reference and digest before saving because protected coordinates are not returned to the browser.'
    : 'No signature image is displayed or stored in this ordinary form.';
  ...
  return `<form class="entry-form cif-draft-form${isEdit ? ' cif-edit-draft' : ' cif-create-draft'}"
    ...
    <div class="section-heading"><div><h3>${isEdit ? 'Edit draft CIF' : 'Create draft CIF'}</h3><p>Ordinary client information only. Verification evidence remains in the restricted boundary.</p></div></div>
    <div class="form-grid">
    ...
    <fieldset><legend>Permanent address</legend>
      <label class="check-row"><input name="same_as_present_address" type="checkbox"${draft.same_as_present_address ? ' checked' : ''} /> Same as present address</label>
      <div class="form-grid">
    ...
    <fieldset><legend>Privacy and client signature reference</legend><div class="form-grid">
      ...
      <label>Restricted signature object reference<input name="client_signature_reference" maxlength="500" placeholder="restricted-signature://…"${signatureRequired} /></label>
      <label>Signature SHA-256<input name="client_signature_digest" minlength="64" maxlength="64" pattern="[0-9a-f]{64}" placeholder="64 lowercase hexadecimal characters"${signatureRequired} /></label>
    </div><p class="meta">${escapeHtml(signatureMessage)}</p></fieldset>
...
export function cifRecordMarkup(record, session) {
  ...
  const verify = draft && hasPermission(session, 'cif.verify')
    ? `<form class="entry-form cif-verify-form" data-cif-id="${escapeHtml(record.cif_id)}" data-expected-updated-at="${escapeHtml(record.updated_at || '')}"><label>Verification note<textarea name="review_note" minlength="3" maxlength="1000" required></textarea></label><button class="button button-secondary" type="submit">Verify frozen source</button></form>`
    : '';
  const activate = draft && record.source_digest && hasPermission(session, 'cif.approve')
    ? `<form class="entry-form cif-activate-form" data-cif-id="${escapeHtml(record.cif_id)}" data-source-digest="${escapeHtml(record.source_digest)}"><label>Approval note<textarea name="review_note" minlength="3" maxlength="1000" required></textarea></label><button class="button button-primary" type="submit">Activate CIF</button></form>`
    : '';
  ...
  return `<article class="list-item cif-record" data-cif-id="${escapeHtml(record.cif_id || '')}">
    ...
    ${draft ? draftFormMarkup({ clientId: record.client_id, record, session }) : ''}
    ${verify}${activate}
  </article>`;
}

function clientResultsMarkup(items) {
...
function reverificationMarkup(items) {
  if (!items.length) return emptyState('No CIF re-verification requirement is recorded.');
  return `<div class="list-stack">${items.map((item) => `<article class="list-item"><div class="section-heading"><div><strong>${escapeHtml(titleCase(item.reason || 'Requirement'))}</strong><div class="meta">Opened ${formatDateTime(item.opened_at)}</div></div>${badge(item.status)}</div><p>${escapeHtml(item.note || '')}</p><span class="meta">Severity: ${escapeHtml(titleCase(item.severity || 'standard'))}</span></article>`).join('')}</div>`;
}

function restrictedRecordForm(context = {}) {
  return `<form class="entry-form restricted-evidence-record" data-cif-id="${escapeHtml(context.cifId || '')}">
...
export function restrictedEvidenceMarkup(items, session, context = {}) {
  ...
  const list = records.length
    ? `<div class="list-stack">${records.map((item) => `<article class="list-item restricted-evidence-row" data-evidence-id="${escapeHtml(item.evidence_id)}"><div class="section-heading"><div><strong>${escapeHtml(titleCase(item.evidence_type || 'Evidence'))}</strong><div class="meta">Checked ${formatDateTime(item.checked_at)}</div></div>${badge(item.review_decision || item.verification_outcome || 'pending')}</div><div class="detail-grid"><div class="detail-item"><span>Masked reference</span><strong>${escapeHtml(item.masked_reference || '—')}</strong></div><div class="detail-item"><span>Retention class</span><strong>${escapeHtml(titleCase(item.retention_class || '—'))}</strong></div><div class="detail-item"><span>Document date</span><strong>${formatDate(item.document_date)}</strong></div><div class="detail-item"><span>Document expiry</span><strong>${formatDate(item.document_expires_at)}</strong></div></div>${canReview && !item.review_decision ? `<form class="entry-form restricted-evidence-review" data-evidence-id="${escapeHtml(item.evidence_id)}"><label>Decision<select name="decision"><option value="approved">Approve</option><option value="rejected">Reject</option></select></label><label>Review note<textarea name="review_note" minlength="3" maxlength="1000" required></textarea></label><button class="button button-secondary" type="submit">Save independent review</button></form>` : ''}</article>`).join('')}</div>`
    : emptyState('No restricted evidence metadata is recorded for this CIF.');
...
export function cifWorkspaceMarkup(session) {
  ...
  const restricted = ['identity_evidence.view', 'identity_evidence.record', 'identity_evidence.review']
...
function clientDetailMarkup(selected, payload, session) {
...
function bindRestrictedForms(context, section, session) {
  ...
  record?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      await loadRestricted(context, section, session);
    } catch (error) {
...
function bindClientActions(context, section, session, selected) {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        await context.api.request(
          ...
          {
            method: cifId ? 'PATCH' : 'POST',
            body: cifId
              ? { expected_updated_at: form.dataset.expectedUpdatedAt, draft: draftBody(form) }
              : draftBody(form),
          },
        );
  ...
  const restrictedLoad = section.querySelector('.restricted-evidence-load');
  restrictedLoad?.addEventListener('click', () => loadRestricted(context, section, session));
}

async function loadClient(context, section, session, selected) {
  const detail = section.querySelector('.cif-client-detail');
  detail.innerHTML = loadingPanel('Loading CIF versions…');
  try {
    const payload = await context.api.request(`/api/v1/management/clients/${encodeURIComponent(selected.client_id)}/cifs`);
    detail.innerHTML = clientDetailMarkup(selected, payload, session);
    const restricted = section.querySelector('.restricted-evidence-content');
    if (restricted) {
      restricted.innerHTML = restrictedToolbar(asArray(payload?.forms));
    }
    bindClientActions(context, section, session, selected);
  } catch (error) {
...
function bindSearch(context, section, session) {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    const results = section.querySelector('.cif-client-results');
    const button = form.querySelector('button[type="submit"]');
    setButtonBusy(button, true, 'Searching…');
    results.innerHTML = loadingPanel('Searching clients…');
    try {
      const clients = asArray(await context.api.request(`/api/v1/management/cif-clients?q=${encodeURIComponent(query)}`));
      results.innerHTML = clientResultsMarkup(clients);
      for (const select of results.querySelectorAll('.cif-select-client')) {
...
export async function mountCifWorkspace(context) {
  ...
  if (section) bindSearch(context, section, context.session);
}

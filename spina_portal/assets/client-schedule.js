...
export function renderClientSchedule(schedule = {}) {
  ...
  return `<div class="notice-card">
    ...
    <div class="loan-meta">
      ...
      <div class="detail-item"><span>Past due</span><strong>${formatAuthoritativeMoney(schedule.past_due_amount)}</strong></div>
    ...
    ${rows.length ? `<div class="table-wrap"><table>
      ...
      <tbody>${rows.map((row) => `<tr>
        ...
        <td>${formatAuthoritativeMoney(row.amount)}</td>
        <td>${badge(row.status || 'scheduled')}</td>
        <td>${formatAuthoritativeMoney(row.details?.remaining_amount)}</td>

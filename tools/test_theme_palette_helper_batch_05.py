#!/usr/bin/env python3
"""Behavior regression test for display-only theme palette helper batch 05."""
...
DEFAULT_MANIFEST = ROOT / "tools/fixtures/theme_palette_helper_batch_05_manifest.json"
DEFAULT_FIXTURE = ROOT / "tools/fixtures/theme_palette_helper_batch_05_behavior.json"
EXPECTED_HELPERS = [
...
def main() -> int:
    ...
    if current != expected:
        ...
        raise SystemExit("Theme palette helper behavior changed")

    print(
        f"Theme palette behavior matches for {len(helper_names := current['helper_names'])} helpers: "
        + ", ".join(helper_names)
    )
    return 0

#!/usr/bin/env python3
"""Behavior regression test for base theme palette batch 11."""
...
DEFAULT_MANIFEST = ROOT / "tools/fixtures/base_theme_palette_batch_11_manifest.json"
DEFAULT_FIXTURE = ROOT / "tools/fixtures/base_theme_palette_batch_11_behavior.json"
EXPECTED_HELPERS = [
...
def main() -> int:
    ...
    if current != expected:
        ...
        raise SystemExit("Base theme palette behavior changed")

    helper_names = current["helper_names"]
    print(
        f"Base theme palette behavior matches for {len(helper_names)} helpers: "
        + ", ".join(helper_names)
    )
    return 0

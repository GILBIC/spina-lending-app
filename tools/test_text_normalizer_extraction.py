#!/usr/bin/env python3
"""Regression checks for the extracted SPINA text normalizers."""
...
from extract_text_normalizers import (
    IMPORT_LINES,
...
DEFAULT_FIXTURE = Path("tools") / "fixtures" / "text_normalizers_behavior.json"
...
def _validate_signature(name: str, function) -> None:
    ...
    if not positional or len(required_positional) > 1 or required_keyword_only:
        raise AssertionError(
            f"Unsupported signature for guarded text-normalizer regression: {name}{signature}"
        )
...
def _functions_from_module_source(module_source: str) -> dict[str, object]:
    ...
    exec(compile(module_source, "<legacy-text-normalizers>", "exec"), namespace)
    return {name: namespace[name] for name in TARGETS}


def _load_generated_functions(module_path: Path) -> dict[str, object]:
    spec = importlib.util.spec_from_file_location("temp_spina_text", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load generated text module.")
...
def main() -> int:
    ...
    fixture_path = (
        Path(args.write_fixture)
        if args.write_fixture
        else repo_root / DEFAULT_FIXTURE
    )
    ...
    if definitions == set(TARGETS) and not imports:
        ...
        if not fixture_path.exists():
            raise AssertionError(f"Missing text-normalizer behavior fixture: {fixture_path}")
        _test_applied_state(repo_root, fixture_path)
    else:
        raise AssertionError(
            f"Unexpected mixed text-normalizer state: definitions={definitions}, imports={imports}"
        )
    return 0

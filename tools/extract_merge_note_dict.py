#!/usr/bin/env python3
"""Guarded extraction for the pure _merge_note_dict helper."""
...
EXPECTED_SIGNATURE = "dst: dict, src: dict"
EXPECTED_EXTERNAL_NAMES = {"_append_unique_text", "_as_note_dict"}
...
def build_plan() -> dict[str, object]:
    ...
    node = app_matches[0] if state == "source" else (module_matches[0] if state == "extracted" else None)
    if node is not None:
        plan.update(
            {
                "line": node.lineno,
...
def validate_plan(plan: dict[str, object]) -> None:
    ...
    if set(plan.get("external_names") or []) != EXPECTED_EXTERNAL_NAMES:
...
def apply_extraction() -> dict[str, object]:
    ...
    patched_module = module_source + function_text.lstrip().rstrip() + "\n"

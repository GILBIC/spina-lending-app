...
TEST_DATABASE_PREFIX = "spina_7x7_eir_anchor_"
BOOTSTRAP_THROUGH = 62
INTEGRATION_TEST = (
    Path(__file__).resolve().parents[1]
    / "gilbic_backend"
    / "tests"
    / "test_7x7_eir_initial_carrying_anchor_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit(
            "7x7 EIR initial-carrying disposable validation refused: integration test file is missing."
        )
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "through 0062, then prove the protected 7x7 original-EIR and initial-gross-"
            "carrying anchor while current carrying, journals and automatic posting stay blocked."
        )
    )
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "7x7 EIR initial-carrying disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(f"7x7 EIR initial-carrying janitor passed: dropped={dropped}.")
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "7x7 EIR initial-carrying validation refused: generated database already exists."
                )
        ...
        if result != 0:
            raise SystemExit(
                "7x7 EIR initial-carrying disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "7x7 EIR initial-carrying disposable PostgreSQL validation passed: "
            "current Management policy evidence was bound to explicit initial-measurement "
            "evidence, original EIR was solved from the exact verified schedule and that "
            "initial carrying amount rather than the operational rate, exact reconciliation "
            "was enforced, and current carrying/journals/automatic posting stayed disabled."
        )
        return 0
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "7x7 EIR initial-carrying disposable PostgreSQL cleanup failed: "

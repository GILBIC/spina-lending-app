...
TEST_DATABASE_PREFIX = "spina_renewal_treatment_decision_"
BOOTSTRAP_THROUGH = 56
INTEGRATION_TEST = (
    Path(__file__).resolve().parents[1]
    / "gilbic_backend"
    / "tests"
    / "test_renewal_treatment_decision_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit(
            "Renewal treatment decision disposable validation refused: integration test file is missing."
        )
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "migrations through 0056, prove immutable Management-reviewed renewal "
            "treatment decision evidence, then remove the database."
        )
    )
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "Renewal treatment decision disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(
            "Renewal treatment decision disposable PostgreSQL janitor passed: "
            f"dropped={dropped}."
        )
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "Renewal treatment decision disposable validation refused: generated database already exists."
                )
        ...
        if result != 0:
            raise SystemExit(
                "Renewal treatment decision disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "Renewal treatment decision disposable PostgreSQL validation passed: "
            "immutable reviewed decisions, exact retry, direct mutation rejection, "
            "explicit void/correction history, renewal-execution history protection, "
            "and forced audit-failure rollback were proven in a local disposable database. "
            "No journal coordinates or automatic classification/posting were enabled."
        )
        return 0
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "Renewal treatment decision disposable PostgreSQL cleanup failed: "

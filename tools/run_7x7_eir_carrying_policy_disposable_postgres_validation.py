...
TEST_DATABASE_PREFIX = "spina_7x7_eir_policy_"
BOOTSTRAP_THROUGH = 60
INTEGRATION_TEST = (
    Path(__file__).resolve().parents[1]
    / "gilbic_backend"
    / "tests"
    / "test_7x7_eir_carrying_policy_readiness_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit(
            "7x7 EIR/carrying policy disposable validation refused: integration test file is missing."
        )
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "migrations through 0060, prove that verified 7x7 base cash flows can "
            "produce only a mathematical no-prepayment EIR preview while authoritative "
            "IFRS 9 EIR/carrying measurement remains fail-closed, then remove the database."
        )
    )
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "7x7 EIR/carrying policy disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(f"7x7 EIR/carrying policy janitor passed: dropped={dropped}.")
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "7x7 EIR/carrying policy validation refused: generated database already exists."
                )
        ...
        if result != 0:
            raise SystemExit(
                "7x7 EIR/carrying policy disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "7x7 EIR/carrying policy disposable PostgreSQL validation passed: "
            "verified signed-contract cash flows solved the base no-prepayment mathematical "
            "daily EIR preview; the operational PHP 7-per-PHP 1,000 rate remained separately "
            "identified; matching base mathematics did not create an authoritative EIR or "
            "carrying amount; business-model/SPPI/prepayment expected-cash-flow gates stayed "
            "explicit; journal lines and automatic source posting stayed disabled."
        )
        return 0
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "7x7 EIR/carrying policy disposable PostgreSQL cleanup failed: "

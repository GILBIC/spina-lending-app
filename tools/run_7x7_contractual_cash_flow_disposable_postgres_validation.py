...
def main() -> int:
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "7x7 contractual cash-flow disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(
            "7x7 contractual cash-flow disposable PostgreSQL janitor passed: "
            f"dropped={dropped}."
        )
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "7x7 contractual cash-flow disposable validation refused: generated database already exists."
                )
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "7x7 contractual cash-flow disposable PostgreSQL cleanup failed: "

...
INTEGRATION_TEST = (
    ...
    / "gilbic_backend"
    / "tests"
    / "test_renewal_boundary_eir_posting_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit(
            "Renewal-boundary EIR disposable validation refused: integration test file is missing."
        )
    env = os.environ.copy()
...
def main() -> int:
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "Renewal-boundary EIR disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(
            "Renewal-boundary EIR disposable PostgreSQL janitor passed: "
            f"dropped={dropped}."
        )
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "Renewal-boundary EIR disposable validation refused: generated database already exists."
                )
        ...
        if result != 0:
            raise SystemExit(
                "Renewal-boundary EIR disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "Renewal-boundary EIR disposable PostgreSQL validation passed: "
            "protected drafts, exact retry, manual mutation/post/reversal rejection, "
            "explicit posting, immutable audit, execution-void guard, and forced "
            "multi-entry rollback were proven in a local disposable database."
        )
        return 0
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "Renewal-boundary EIR disposable PostgreSQL cleanup failed: "

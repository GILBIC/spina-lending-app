...
TEST_DATABASE_PREFIX = "spina_7x7_journal_draft_"
BOOTSTRAP_THROUGH = 64
INTEGRATION_TEST = (
    Path(__file__).resolve().parents[1]
    / "gilbic_backend"
    / "tests"
    / "test_7x7_protected_journal_draft_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit("7x7 protected journal-draft validation refused: integration test file is missing.")
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "through 0064, then prove the Management-confirmed stale-safe 7x7 journal-draft "
            "boundary without posting, reversal or automatic source posting."
        )
    )
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit("7x7 journal-draft validation refused: configured database uses the reserved disposable prefix.")
    ...
    if args.cleanup_stale_only:
        ...
        print(f"7x7 journal-draft janitor passed: dropped={dropped}.")
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit("7x7 journal-draft validation refused: generated database already exists.")
        ...
        if result != 0:
            raise SystemExit(
                "7x7 protected journal-draft disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "7x7 protected journal-draft disposable PostgreSQL validation passed: "
            "exact 0064 review-token and coordinate-digest confirmation created one immutable "
            "balanced draft; exact retry was idempotent; stale/tampered confirmation failed closed; "
            "General Journal editing was blocked; posting, reversal and automatic source posting stayed disabled."
        )
        return 0
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "7x7 protected journal-draft disposable PostgreSQL cleanup failed: "

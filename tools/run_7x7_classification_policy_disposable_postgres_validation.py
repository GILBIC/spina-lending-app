...
TEST_DATABASE_PREFIX = "spina_7x7_policy_evidence_"
BOOTSTRAP_THROUGH = 61
INTEGRATION_TEST = (
    Path(__file__).resolve().parents[1]
    / "gilbic_backend"
    / "tests"
    / "test_7x7_classification_policy_evidence_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit(
            "7x7 classification policy disposable validation refused: integration test file is missing."
        )
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "through 0061, then prove the protected 7x7 Management classification / expected-"
            "cash-flow policy evidence boundary while authoritative EIR, carrying and posting "
            "remain fail-closed."
        )
    )
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "7x7 classification policy disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(f"7x7 classification policy janitor passed: dropped={dropped}.")
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "7x7 classification policy validation refused: generated database already exists."
                )
        ...
        if result != 0:
            raise SystemExit(
                "7x7 classification policy disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "7x7 classification policy disposable PostgreSQL validation passed: "
            "Management-only immutable business-model/SPPI/measurement-category and expected-"
            "life/prepayment policy evidence was stale-safe and idempotent; unsupported or "
            "evidence-incomplete states remained blocked; authoritative EIR/carrying amounts, "
            "journal lines and automatic source posting stayed disabled."
        )
        return 0
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "7x7 classification policy disposable PostgreSQL cleanup failed: "

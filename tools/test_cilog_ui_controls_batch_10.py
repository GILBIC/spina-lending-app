#!/usr/bin/env python3
"""Headless behavior regression test for Client Information Log UI controls."""
...
DEFAULT_MANIFEST = ROOT / "tools/fixtures/cilog_ui_controls_batch_10_manifest.json"
DEFAULT_FIXTURE = ROOT / "tools/fixtures/cilog_ui_controls_batch_10_behavior.json"
...
def _cilog_palette(_self: Any = None) -> dict[str, str]:
    if _palette_raises:
...
def _capture_button(function: Callable[..., Any], case: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(result, FakeWidget):
        raise RuntimeError(f"Expected a widget result, got {result!r}")
    return {
        "kind": "return",
        "tree": _widget_summary(parent),
        "returned": _widget_summary(result),
        "identity": {
            "button_is_root_first_child": bool(parent.children and result is parent.children[0]),
        },
    }
...
def capture_batch(manifest_path: Path) -> dict[str, Any]:
    ...
    cases = {
        "_spina_v24_cilog_button": [
            {"name": "normal_defaults", "args": ["Refresh"]},
            {"name": "primary", "args": ["Apply", "apply-command", "primary"]},
            {"name": "success_width", "args": ["Approve", "approve-command", "success", 16]},
            {"name": "danger", "args": ["Delete", "delete-command", "danger"]},
            {"name": "soft", "args": ["Close", None, "soft"]},
            {"name": "palette_failure", "args": ["Refresh"], "palette_raises": True},
        ],
        "_spina_v24_cilog_style_tree": [
            {"name": "normal"},
    ...
    for helper in manifest["helpers"]:
        ...
        capture = _capture_button if name.endswith("cilog_button") else _capture_style
        behavior[name] = {
...
def main() -> int:
    ...
    if current != expected:
        ...
        raise SystemExit("Client Information Log UI-control behavior changed")

    print(
        f"Client Information Log UI-control behavior matches for {len(current['helper_names'])} helpers: "

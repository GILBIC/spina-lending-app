...
TEST_DATABASE_PREFIX = "spina_ecl_methodology_"
BOOTSTRAP_THROUGH = 68
INTEGRATION_TEST = (
    Path(__file__).resolve().parents[1]
    / "gilbic_backend"
    / "tests"
    / "test_ecl_methodology_source_policy_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit(
            "ECL methodology/source-policy validation refused: integration test file is missing."
        )
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "through 0068, then apply and prove the fail-closed 0069 ECL methodology/source policy."
        )
    )
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "ECL methodology/source-policy validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(f"ECL methodology/source-policy janitor passed: dropped={dropped}.")
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "ECL methodology/source-policy validation refused: generated database already exists."
                )
        ...
        if result != 0:
            raise SystemExit(
                "ECL methodology/source-policy disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "ECL methodology/source-policy disposable PostgreSQL validation passed: "
            "0069 approved the evidence classes and probability-weighted discounted cash-shortfall "
            "boundary while numeric PD/LGD/cure/recovery/scenario assumptions, automatic staging, "
            "ECL calculation, account 1190 posting and automatic source posting remained disabled."
        )
        return 0
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "ECL methodology/source-policy disposable PostgreSQL cleanup failed: "

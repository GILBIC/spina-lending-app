...
def main() -> int:
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "Remittance transfer journal disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(
            "Remittance transfer journal disposable PostgreSQL janitor passed: "
            f"dropped={dropped}."
        )
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "Remittance transfer journal disposable validation refused: generated database already exists."
                )
        ...
        print(
            ...
            "installation created no financial history; exact reviewed Dr destination "
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "Remittance transfer journal disposable PostgreSQL cleanup failed: "

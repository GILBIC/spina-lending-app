...
WORKFLOW_PATH = ".github/workflows/zz_tmp_no_collection_fixups.yml"
HELPER_PATH = "tools/tmp_patch_no_collection_fixups.py"
...
def request_json(method: str, url: str, payload: dict[str, object] | None = None) -> dict[str, object]:
    headers = {
        ...
        "User-Agent": "spina-no-collection-fixups",
    }
...
def main() -> None:
    ...
    text, sha = load(path)
    if "balance.due_date" not in text:
        if "balance.effective_due_date" not in text:
            raise RuntimeError("Collector route operational date alias target not found")
    else:
        text = text.replace("balance.due_date", "balance.effective_due_date")
        save(path, text, sha, "Backend: fix Collector next effective due-date alias")
    ...
    if 'row["due_date"]' in text:
        ...
        save(path, text, sha, "Backend: read effective due-date fields after No Collection")
    elif 'row["effective_due_date"]' not in text:
        raise RuntimeError("Contract posting effective due-date row target not found")

    delete(WORKFLOW_PATH, "CI: remove temporary No Collection fixup workflow")
    delete(HELPER_PATH, "CI: remove temporary No Collection fixup helper")

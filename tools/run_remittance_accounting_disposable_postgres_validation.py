...
TEST_DATABASE_PREFIX = "spina_remittance_accounting_"
BOOTSTRAP_THROUGH = 56
INTEGRATION_TEST = (
    Path(__file__).resolve().parents[1]
    / "gilbic_backend"
    / "tests"
    / "test_remittance_accounting_evidence_postgres.py"
)
...
def _run_test(test_database_url: str) -> int:
    if not INTEGRATION_TEST.is_file():
        raise SystemExit(
            "Remittance accounting disposable validation refused: integration test file is missing."
        )
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "migrations through 0056, prove explicit remittance destination evidence "
            "and asset-to-asset custody-transfer coordinates, then remove the database."
        )
    )
    ...
    if configured_database.startswith(TEST_DATABASE_PREFIX):
        raise SystemExit(
            "Remittance accounting disposable validation refused: configured database uses the reserved disposable prefix."
        )
    ...
    if args.cleanup_stale_only:
        ...
        print(
            "Remittance accounting disposable PostgreSQL janitor passed: "
            f"dropped={dropped}."
        )
        return 0
    ...
    try:
        with psycopg.connect(admin_url, autocommit=True) as admin:
            if disposable._database_exists(admin, test_database):
                raise SystemExit(
                    "Remittance accounting disposable validation refused: generated database already exists."
                )
        ...
        if result != 0:
            raise SystemExit(
                "Remittance accounting disposable PostgreSQL validation failed: "
                f"integration test exited with code {result}."
            )
        print(
            "Remittance accounting disposable PostgreSQL validation passed: received "
            "custody evidence was required, Office/Bank-GCash destination evidence was "
            "explicit and immutable/idempotent, Dr destination cash / Cr Collector "
            "Custody coordinates were balanced, no income was recognized, correction "
            "required explicit void, journal history protected evidence, and automatic "
        ...
        if cleanup_required:
            try:
                ...
                message = (
                    "Remittance accounting disposable PostgreSQL cleanup failed: "

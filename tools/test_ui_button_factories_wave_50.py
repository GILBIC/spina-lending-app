...
RETIRED = "_spina_v25_collector_button"
TARGETS = (
...
EXPECTED_SIGNATURES = {
    name: "parent, text, command=None, kind='normal', width=None" for name in TARGETS
}
EXPECTED_CALLS = {
...
def verify_function(path: Path, name: str, expected: str) -> ast.FunctionDef:
...
def main() -> None:
    ...
    assert not [node for node in ui_tree.body if isinstance(node, ast.FunctionDef) and node.name == RETIRED]
    ...
    assert ("_spina_v25_collector_colors", None) not in aliases
    ...
    assert RETIRED not in imported_names
    assert set(TARGETS).issubset(imported_names)

    for (path, name, _mode), expected in PROTECTED_CALLERS.items():
        node = verify_function(path, name, expected)
        expected_target = {
            "_spina_v27_build_collectors_tab": "_spina_v27_route_button",
            "_spina_v32_prompt_login": "_spina_v32_login_button",
        }[name]
        assert expected_target in calls_for(node), (name, expected_target)

    for name, expected in PALETTE_HASHES.items():
        verify_function(THEME_PALETTES, name, expected)

    print("Wave 50 UI button-factory regression passed: 2 active factories, 58 lines; obsolete v25 factory retired.")

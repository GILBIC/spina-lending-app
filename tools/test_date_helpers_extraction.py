#!/usr/bin/env python3
"""Regression checks for the three extracted SPINA date helpers.

Before extraction, this test applies the guarded move to a temporary copy and
compares behavior. After extraction, it checks the committed behavior fixture,
module imports, and absence of the old top-level definitions.
"""
...
from extract_date_helpers_module import (
    IMPORT_LINES,
...
def _validate_signature(name: str, function) -> None:
    ...
    if not positional or len(required_positional) > 1 or required_keyword_only:
        raise AssertionError(
            f"Unsupported signature for guarded date-helper regression: {name}{signature}"
        )
...
def _functions_from_module_source(module_source: str) -> dict[str, object]:
    ...
    exec(compile(module_source, "<legacy-date-helpers>", "exec"), namespace)
    return {name: namespace[name] for name in TARGETS}


def _load_generated_functions(module_path: Path) -> dict[str, object]:
    spec = importlib.util.spec_from_file_location("temp_spina_dates", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load generated dates module.")

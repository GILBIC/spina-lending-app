#!/usr/bin/env python3
"""Guarded extraction for the pure _append_unique_text helper."""
...
EXPECTED_SIGNATURE = "existing: str, addition: str"
...
def external_names(node: ast.FunctionDef) -> set[str]:
    ...
    return loaded - local_names - set(dir(builtins)) - {TARGET}
...
def apply_extraction() -> dict[str, object]:
    ...
    function_text = "".join(app_lines[node.lineno - 1:node.end_lineno])
    if not function_text.endswith("\n"):
        function_text += "\n"

    app_lines[node.lineno - 1:node.end_lineno] = [IMPORT_LINE + "\n"]
    patched_app = "".join(app_lines)

...
Management is also intended to review what Employees prepare and complete through a permission-scoped Employee Activity workspace. That view links to authoritative Accounting, HR, Payroll, CRM/support, remittance/operations, and administration evidence only when Management has the owning domain permission. It does not authorize impersonation, silent draft editing, surveillance, or maker-checker bypass. This workspace is planned behavior, not current implemented behavior.

...
1. **PR #225:** finish owner-only Flutter validation for the guarded collector Payment/ADV/PASS form.
2. Merge only after Flutter dependency resolution, `flutter analyze --fatal-infos`, tests, clean-tree checks, and safe manual verification pass.
...
| Collector entry form | **In progress** | Implemented on draft PR #225 with online-only safety gates and safe manual retry | Flutter validation is queued; PR remains unmerged |
...
| #225 | Guarded collector Payment/ADV/PASS form | **Open draft; validation queued** |
...
- PR #225 checks pass on the exact head.
